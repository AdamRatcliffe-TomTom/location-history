...
// node_modules/valvelet/index.js
var require_valvelet = __commonJS({
  "node_modules/valvelet/index.js"(exports, module2) {
    "use strict";
    function valvelet2(fn, limit, interval, size) {
      const queue = [];
      let count = 0;
...
// services/objects.js
var import_valvelet = __toESM(require_valvelet());
...
var addObject = async ({ apiKey, adminKey, object }) => {
  const url2 = `${LOCATION_HISTORY_API_BASE_URL}/objects?key=${apiKey}&adminKey=${adminKey}`;
  const { data } = await axios_default.post(url2, object);
  return data;
};
var editObject = async ({ apiKey, id, object }) => {
  const url2 = `${LOCATION_HISTORY_API_BASE_URL}/objects/${id}?key=${apiKey}`;
  const { data } = await axios_default.put(url2, object);
  return data;
};
var getObjects = async ({ apiKey }) => {
  const url2 = `${LOCATION_HISTORY_API_BASE_URL}/objects?key=${apiKey}`;
  const { data } = await axios_default.get(url2);
...
var deleteObject = async ({ apiKey, id }) => {
  const url2 = `${LOCATION_HISTORY_API_BASE_URL}/objects/${id}?key=${apiKey}`;
  const { data } = axios_default.delete(url2);
  return data;
};
